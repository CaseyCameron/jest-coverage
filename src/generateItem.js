import { getRandomInt, getRandomIntInRange }  from "./utils/helper";

const DAMAGE_TYPES = ["piercing", "slashing", "bludgeoning"];
const ARMOR_TYPES = ["light", "medium", "heavy"];

export function generateStat(rarity) {
  let stat = 0;
  if (rarity === "epic") {
...
export function generateBaseWeapon() {
  return {
    type: "weapon",
...
export function generateBaseArmor() {
  return {
    type: "armor",
    defense: 0,
...
export function generateItem(itemOptions = {}) {
